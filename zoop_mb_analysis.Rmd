...
diversity_values <- richness.plot.location$data[252:502,]
richness_values <- richness.plot.location$data[1:251,]
lake_rich <- aggregate(value ~ lake, data = richness_values, FUN = function(x) c(mean = mean(x), sd = sd(x)))
#lake_rich <- summaryBy(value ~ lake, data = richness_values, FUN = c(mean, sd))
p <- ggplot(lake_rich, aes(x = lake, y = value[,"mean"])) +
  geom_boxplot() +
  geom_errorbar(aes(ymin = value[,"mean"] - value[,"sd"], ymax = value[,"mean"] + value[,"sd"]),
                width = 0.2, color = "red") +
  labs(title = "Richness by Lake", x = "Lake", y = "Richness") +
  theme_minimal()
# Perform Tukey's test
tukey <- TukeyHSD(aov(value ~ lake, data = richness_values))
tukey_sig <- data.frame(tukey$lake[, "p adj"])
tukey_sig <- subset(tukey_sig, tukey.lake....p.adj.. < 0.05)


# Add Tukey test symbols to the plot
richness.plot.location +
  geom_text(data = tukey_sig,
            aes(label = ifelse(tukey.lake....p.adj.. < 0.05, "*", ""), y = max(richness_values$value), x = richness_values$lake),
            vjust = -1, size = 6)

richness.plot.location + stat_compare_means(comparisons = tukey_sig, label = "*")

# Check if there are any significant comparisons
if (any(tukey$lake[, "p adj"] < 0.05)) {
  # Extract the significant comparisons and p-values
  tukey_sig <- data.frame(tukey$lake[, "p adj"])
  tukey_sig <- subset(tukey_sig, tukey.lake....p.adj.. < 0.05)
  
  # Add the significance labels to the plot
  p +
    geom_text(data = tukey_sig,
              aes(x = as.numeric(rownames(tukey_sig)), y= lake_rich$value[,"mean"] + lake_rich$value[,"sd"] + 0.2, label = "*"),
              size = 4, vjust = 0) + theme_minimal()
} else {
  p
}
...
rare_div_lm <- lm(shannon ~ lake, data = rare.a.diversity)
rare_anova_div <- aov(rare_div_lm)

rare.sum_rich <- summaryBy(observed ~ lake, data = rare.a.diversity, FUN = c(mean, sd))
rare_rich_lm <- lm(observed ~ lake, data = rare.a.diversity)
rare_anova_rich <- Anova(rare_rich_lm)

# Tukey test
rare.tukey.test <- TukeyHSD(rare_anova_div, conf.level=0.95)
...
Anova(lm(shannon ~ lake*time_point, data = a.diversity))
...
water.a.diversity <- cbind(water.a.diversity, sample_data(PS.water)[,c(4,6,8,9)])
...
water_div_lm <- lm(shannon ~ lake*time_point, data = water.a.diversity)
Anova(water_div_lm)
water_rich_lm <- lm(observed ~ lake*time_point, data = water.a.diversity)
Anova(water_rich_lm)
...
barchart = bargraph.CI(basin, shannon, data = rare.a.diversity,
                     xlab = "Treatment", ylab = "grams live biomass", cex.lab = 1.5, x.leg = 2,
...
broad.model
adonis2(bc ~ sample_type, data = nmds.merge1, permutations = 1000)
...
broad.model3
adonis2(bc_zoops ~ DOC, data = nmds.merge3, permutations = 1000)
...
ggplot(NMDSxPCA_data, aes(x = NMDS1, y = PC1, color = lake)) +
  geom_point() +  
  geom_smooth(aes(color=lake, fill=lake), method = "lm", alpha = .2, size=0.6) +
  xlab("NMDS (Bray-Curtis") + ylab("PC1 (Environment)") +
  theme_bw()
...
spatial_dist <- dist(gps, method = "euclidean")
geodists_km <- earth.dist(gps[,1:2], dist = T)
dists_m <- as.dist(geodists_km * 1000)
#dists_m <- as.matrix(dists_m)
rownames(dists_m) <- rownames(gps$V)
colnames(dists_m) <- t(rownames(sample_data(PS.fin)))
rownames(dists_m) <- colnames(dists_m) <- rownames(sample_data(PS.fin))

bc_dist =vegdist(otu_table(PS.fin), "bray")
study_bc_mat<-as.matrix(vegdist(otu_table(PS.fin), "bray"))

study_mantel_test<-mantel.rtest(dists_m, study_bc_dist, nrepet=9999)
study_mantel_test
mantel_test <- mantel(dists_m ~ bc_dist, nperm = 999)
...
mantel_test <- mantel(bc ~ mat_geodists_km)
...
ggplot(glom_melt, aes(x = time_point, y = Abundance))
...
glom_plot
dev.copy(pdf, "phyloseq analysis/figures/glom_plot.pdf", height=8, width=11)
dev.off()
...
######
#Testing something from chatGPT to plot just the rare taxa of bacteria
# Set the abundance threshold for rare taxa (adjust as needed)
abundance_threshold <- 0.05  # Only taxa with abundance less than 1% will be considered rare

# Calculate the relative abundance of taxa
relative_abundance <- transform_sample_counts(PS.500, function(x) x / sum(x))
abundance_matrix <- otu_table(relative_abundance)


# Identify the rare taxa based on the abundance threshold
rare_taxa <- tax_table(PS.500)[rowSums(abundance_matrix) < abundance_threshold, ]
abundance_sums <- rowSums(abundance_matrix)
rare_taxa_sums <- abundance_sums[rowSums(abundance_matrix) < abundance_threshold]

# Create a bar plot for the rare taxa by ASV
barplot(rare_taxa_sums, 
        main = "Rare Taxa Abundance",
        xlab = "Taxa",
        ylab = "Relative Abundance")

# Set the taxonomic level of interest (class in this case)
taxonomic_level <- "Class"

tax_table <- tax_table(PS.500)

# Get the abundance matrix
abundance_matrix <- as.matrix(otu_table(PS.500))

# Identify the rare taxa at the taxonomic level of interest
rare_taxa_class <- rare_taxa[taxa_names(rare_taxa) %in% taxa_names(tax_table), ]

# Get the corresponding taxonomic ranks for the rare taxa
rare_taxa_ranks <- tax_table(rare_taxa_class)[, taxonomic_level]

# Subset the rare taxa to include only the taxonomic level of interest (class)
rare_taxa_class <- rare_taxa_class[!is.na(rare_taxa_ranks), ]

# Get the corresponding abundance matrix for rare taxa at the class level
abundance_matrix_class <- abundance_matrix[row.names(abundance_matrix) %in% taxa_names(rare_taxa_class), ]

# Calculate the sums of relative abundance for the rare taxa at the class level
abundance_sums_class <- rowSums(abundance_matrix_class)

# Create a bar plot for the rare taxa at the class level
barplot(abundance_sums_class,
        main = "Rare Taxa Abundance at Class Level",
        xlab = "Taxa",
        ylab = "Relative Abundance")
...
Abund <- aggregate(Abundance ~ sequence_ID + time_point + lake + organism_water + phy_group + 
...
phy_group_gen <- ggplot(Abund, aes(x = phy_group, y = Abundance, fill= Order, color = Order)) +
...
zoops.v.water <- ggplot(Abund500, aes(x = sample_type, y = Abundance), fill = Order, color = Order) +
  geom_bar(aes(fill = Order, color = Order), stat ="identity", position ="fill", width = 0.85) +
  xlab("Sample Type") + theme(axis.text.x=element_text(size=20)) + theme(strip.text.x = element_text(size = 20)) + theme_bw() 
zoops.v.water
dev.copy(pdf, "phyloseq analysis/figures/zoops.v.water.pdf", height=5, width=8)
dev.off() 
