...
set.seed(10000)
hell.model.env <- adonis2(bc.hell ~ temp_C + DOC, data = nmds.merge.hell, permutations = 1000)
hell.model.env
...
# why aren't these average relative abundances of the different classes of bacteria not reflecting what's in the stacked bar plots?:
# one way
Abund.class <- aggregate(Abundance ~ Class, data=Abund.hell, FUN=mean)

# another way
sum.abund <- summaryBy(Abundance ~ Class*sample_type, data = Abund.hell, FUN = mean)

# another method, but does not seem to work either
summary_by_class <- tapply(Abund.hell$Abundance, Abund.hell$Class, FUN = mean)
...
# plots
overall.comp.hell <- ggplot(Abund.hell, aes(x=1, y=Abundance, fill=Class, color=Class)) +
  geom_bar(aes(fill=Class, color=Class), stat="identity", position="fill") +
