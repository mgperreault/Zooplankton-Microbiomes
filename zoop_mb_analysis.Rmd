...
```{r testing differences between zoops and water}
PS.fin.zoops <- subset_samples(PS.fin, sample_type == "Zooplankton")
PS.hell.zoops = transform_sample_counts(PS.fin.zoops, function(x) x^0.5)

bc.hell.zoops <- phyloseq::distance(PS.hell.zoops, method = "bray")

set.seed(10000)
nmds.hell.zoops <- metaMDS(bc.hell.zoops,
          distance = "bray",
          k = 3,
          maxit = 999, 
          wascores = TRUE)
stress_result.zoops <- nmds.hell.zoops$stress
species.scores.hell.zoops <- as.data.frame(scores(nmds.hell.zoops))
nmds.merge.hell.zoops <- merge(sample_data(PS.hell.zoops), species.scores.hell.zoops, by = 'row.names', all = TRUE)


# estimate the actual statistical significance of differences in the observed community composition between two groups of samples.
set.seed(10000)
hell.model.zoops <- adonis2(bc.hell.zoops ~ lake*time_point, data = nmds.merge.hell.zoops, permutations = 1000)
hell.model.zoops


set.seed(10000)
hell.model.env.zoops <- adonis2(bc.hell.zoops ~ temp_C, data = nmds.merge.hell.zoops, permutations = 1000)
hell.model.env.zoops

#######

PS.fin.water <- subset_samples(PS.fin, sample_type == "Water")
PS.hell.water = transform_sample_counts(PS.fin.water, function(x) x^0.5)

bc.hell.water <- phyloseq::distance(PS.hell.water, method = "bray")

set.seed(10000)
nmds.hell.water <- metaMDS(bc.hell.water,
          distance = "bray",
          k = 3,
          maxit = 999, 
          wascores = TRUE)
stress_result.water <- nmds.hell.water$stress
species.scores.hell.water <- as.data.frame(scores(nmds.hell.water))
nmds.merge.hell.water <- merge(sample_data(PS.hell.water), species.scores.hell.water, by = 'row.names', all = TRUE)


# estimate the actual statistical significance of differences in the observed community composition between two groups of samples.
set.seed(10000)
hell.model.water <- adonis2(bc.hell.water ~ time_point, data = nmds.merge.hell.water, permutations = 1000)
hell.model.water


set.seed(10000)
hell.model.env.water <- adonis2(bc.hell.water ~ temp_C, data = nmds.merge.hell.water, permutations = 1000)
hell.model.env.water
...
Anova(lm(Shannon ~ lake, data = a.diversity))
...
# run the Mantel test
mantel <-vegan::mantel(spatial_dist_km, bc.hell)
...
nmds.time <- ggplot(nmds.merge.hell, aes(x=NMDS1, y=NMDS2, color = time_point)) +
  geom_point(aes(color = time_point), size = 3) + 
  stat_ellipse(level=0.9, lwd = 1.2, linetype = 2, aes(color=time_point)) +
  scale_color_manual(values = c("blue","red", "purple2", "turquoise", "orange2")) + 
  theme_bw() +
