...
#richness by Lake
richness.plot.location <- plot_richness(PS.fin, x="lake", measures=c("Shannon")) + 
  aes(color = sample_type) +
  labs(y= "Shannon Diversity", x = "Lake") + theme_bw() + geom_boxplot() + 
  theme(axis.text.x=element_text(size=16)) + theme(axis.title.x=element_text(size=16)) +
  theme(axis.text.y=element_text(size=16)) + theme(axis.title.y=element_text(size=16)) +
  theme(strip.text.x = element_text(size = 20))
richness.plot.location + geom_point(aes(color = sample_type), position = position_jitterdodge(jitter.width = 0.1, dodge.width = 0.8))
...
s.div.lake <- ggplot(a.diversity, aes(x = lake, y = Shannon, color = sample_type)) +
  ...
  theme(strip.text.x = element_text(size = 20))
s.div.lake <- s.div.lake + scale_color_manual(values=c("#56B4E9", "#E69F00"),
                       name="Sample Type",
...
tukey <- TukeyHSD(shan.anova) # shows that Serene is highly different from all the other lakes and is driving the significant difference
cld <- multcompLetters4(shan.anova, tukey)

# Testing effects on observed richness
anova(lm(Observed ~ time_point, data = a.diversity))
rich.anova <- aov(lm(Observed ~ time_point, data = a.diversity))
tukey <- TukeyHSD(rich.anova) # again, Serene is driving the significant difference
cld <- multcompLetters4(rich.anova, tukey)

# testing effects of environmental variables on Shannon diversity and ASV richness
...
rich.mod <- summary(lm(Observed ~ DOC + elevation_m + chla_ug.L + DO_perc + cond, data = a.diversity))
microbe_rich_DOC <- ggplot(a.diversity, aes(x=DOC, y=Observed)) +
    geom_point() +
    geom_smooth(method=lm)
microbe_rich_DOC
dev.print(pdf, "phyloseq analysis/figures/microbe_rich_DOC.pdf", height=10, width=15)
dev.off
DOC_rich <- ggscatter(a.diversity, x = "DOC", y = "Observed",
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "DOC", ylab = "ASV richness")
DOC_rich
dev.print(pdf, "phyloseq analysis/figures/DOC_rich.pdf", height=10, width=15)
dev.off
...
time_rich <- summaryBy(Observed ~ time_point, data = a.diversity, FUN = c(median, mean, se))
...
summary(lm(Abundance.sum ~ chla_ug.L + elevation_m, data = gammaproteobacteria))
gamma_chla <- ggplot(gammaproteobacteria, aes(x=chla_ug.L, y=Abundance.sum)) +
  geom_point() +
  geom_smooth() + xlab("Chlorophyll (ug/L)") + ylab("Gammaproteobacteria Abundances")
gamma_chla
...
Anova(lm(Abundance.sum ~ lake + time_point + phy_group, data = alphaproteobacteria)) #lake, time, and sample type
TukeyHSD(aov(lm(Abundance.sum ~ lake + time_point + phy_group, data = alphaproteobacteria)))
...
actino_DO <- ggplot(actinobacteria, aes(x=DO_perc, y=Abundance.sum)) +
  geom_point() +
  geom_smooth() + xlab("Dissolved Oxygen (%)") + ylab("Actinobacteria Abundances")
actino_DO 
...
stepAIC(FitAll.cyano, direction = "both") # chlorophyll
summary(lm(Abundance.sum ~ chla_ug.L, data = cyanobacteriia)) # nah


cyano_chla <- ggplot(cyanobacteriia, aes(x=chla_ug.L, y=Abundance.sum)) +
  geom_point() +
  geom_smooth() + xlab("Chlorophyll (ug/L)") + ylab("Cyanobacteria Abundances")
cyano_chla
dev.copy(pdf, "phyloseq analysis/figures/actino_DO.pdf", height=6, width=10)
dev.off()
...
time_colors <- c("blue","red", "purple2", "turquoise", "lightpink")
...
time.class <- ggplot(Abund.final, aes(x=time_point, y=rel_abund, fill=Class, color=Class)) +
  geom_bar(aes(fill=Class, color=Class), stat="identity", position="fill") +
  ylab("Relative Abundance (%)") + theme(axis.text.x=element_text(size=12)) +
  theme(axis.text.y=element_text(size=10)) +
  theme(strip.text.y = element_text(size = 15)) + theme_bw() +
