...
OTU <- otu_table(C.abund)
TAX <- tax_table(C.abund)[,"Class"]
AverageD <- as.data.frame(rowMeans(OTU))
names(AverageD) <- c("Mean")
GTable <- merge(TAX, AverageD, by=0, all=TRUE)
sort(GTable[,2], decreasing = T)
...
Cglom = tax_glom(PS.hell, "Family", NArm = F) %>% transform_sample_counts(function(x) {x * 100/sum(x)})
...
df = data.frame(Family = tax_table(Cglom)[,"Family"], Mean = rowMeans(otu_table(Cglom)), row.names = NULL)
df = df[order(-df$Mean),]
head(df)
...
#
names(sort(taxa_sums(PS.hell), TRUE)[1:5])
...
# R squared
summary(lm(observed ~ chla_ug.L, data = rare.a.diversity))
...
rich.mod <- lm(observed ~ chla_ug.L, data = rare.a.diversity)
...
avg.abund <- summaryBy(Abundance ~ Class, data = Abund.hell, FUN = mean)
...
Facet.hell <- ggplot(Abund.hell, aes(x = time_point, y = Abundance, fill= Class, color = Class)) +
  geom_bar(aes(fill = Class, color = Class), stat ="identity", position = "fill") +
  facet_grid(cols = vars(lake)) + xlab("Sampling Time Point") +
...
time.hell <- ggplot(Abund.hell, aes(x=time_point, y=Abundance, fill=Class, color=Class)) +
  geom_bar(aes(fill=Class, color=Class), stat="identity", position="fill") +
  xlab("Sampling Time Point") + theme(axis.text.x=element_text(size=12)) +
  theme(axis.text.y=element_text(size=10)) +
...
sample.type.hell <- ggplot(Abund.hell, aes(x=phy_group, y=Abundance, fill=Class, color=Class)) +
  ...
  xlab("Sample Type") + theme(axis.text.x=element_text(size=12)) +
  theme(axis.text.y=element_text(size=10)) +
  theme(strip.text.y = element_text(size = 15)) + theme_bw() +
  scale_fill_manual(values = c("red4", "orangered", "gold", "lightgreen", "turquoise3", "seagreen4", "lightskyblue", "purple3", "skyblue4", "navy", "tan3", "royalblue3", "black", "antiquewhite2", "chartreuse3", "chocolate", "orchid", "lightpink")) +
  scale_color_manual(values = c("red4", "orangered", "gold", "lightgreen", "turquoise3", "seagreen4", "lightskyblue", "purple3", "skyblue4", "navy", "tan3", "royalblue3", "black", "antiquewhite2", "chartreuse3", "chocolate", "orchid", "lightpink"))
