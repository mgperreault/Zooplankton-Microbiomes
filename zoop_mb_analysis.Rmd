...
```{r aggregate}
str(metadata)
aggregate.data.frame(x = metadata, by = list(phy_group, time_point, lake), FUN = mean)
...
table(metadata_500$phy_group, metadata_500$time_point, metadata_500$lake)

# just a try, rarefying to 1000 reads
set.seed(10000)
PS.1000 <- rarefy_even_depth(PS.fin, rngseed=1e4, sample.size = 1000)
sort(rowSums(otu_table(t(PS.1000))))

# filtering metadata by sample names with 500 reads or more, 49 samples lost
metadata_1000 <- as.data.frame(sample_data(PS.1000))
metadata_1000$sequence_ID <- rownames(PS.1000) 
table(metadata_1000$phy_group, metadata_1000$time_point, metadata_1000$lake)
...
rich_rare <- richness(PS.rare, index="observed")
div_rare <- diversity(PS.rare, index="shannon")
div_rare <- cbind(rich_rare, div_rare)
div_rare$sequence_ID <- rownames(div_rare)

div_rare <- merge(div_rare, metadata_rare[,c(1,7,8,9,20)], by="sequence_ID", all=T)
...
bc_1000 <- phyloseq::distance(PS.1000, method = "bray")
...
nmds2.merge <- merge(sample_data(PS.fin), species.scores, by = 'row.names', all = TRUE)
stressplot(nmds)

nmds2.merge$basin <- as.factor(ifelse(nmds2.merge$lake == "Eastern Brook" | nmds2.merge$lake == "Serene" | nmds2.merge$lake == "Convict", "South", "North"))


# # comparing output using the bray curtis matrix from phyloseq::distance
broad.model <- adonis2(bc_vegdist ~ lake*time_point*phy_group, data = nmds2.merge, permutations = 1000)
broad.model

# comparing output using the bray curtis matrix from avgdist
broad.model <- adonis2(bc_vegdist ~ lake*time_point + phy_group, data = nmds2.merge, permutations = 1000)
broad.model
...
env <- metadata_rare[ ,c(8:15,17,18)]
env <- sample_data(PS.fin)[ ,c(10:19)]


# factors, categorical
facs <- sample_data(PS.fin)[,c(7,9)]
fit.env <- envfit(nmds, env, na.rm = T)
data.scores <- as.data.frame(scores(nmds))
data.scores <- cbind(facs, data.scores)
...
envfit_plot = ggplot(data = data.scores, aes(x = NMDS1, y = NMDS2)) + 
     geom_point(data = data.scores, aes(colour = lake), size = 3, alpha = 0.5) + 
...
test_nmds1 <- ggplot(nmds2.merge[(nmds2.merge$time_point == "1"), ], aes(x=NMDS1, y=NMDS2, color = basin)) +
  geom_point(aes(color = basin, shape = phy_group), size = 3) + 
  stat_ellipse(level=0.9, linetype = 2, aes(color = basin)) +
  theme_bw()
test_nmds1

test_nmds2 <- ggplot(nmds2.merge[(nmds2.merge$time_point == "2"), ], aes(x=NMDS1, y=NMDS2, color = basin)) +
   geom_point(aes(color = basin, shape = phy_group), size = 3) + 
   stat_ellipse(level=0.9, linetype = 2, aes(color = basin)) +
   theme_bw()
test_nmds2

test_nmds3 <- ggplot(nmds2.merge[(nmds2.merge$time_point == "3"), ], aes(x=NMDS1, y=NMDS2, color = basin)) +
  geom_point(aes(color = basin, shape = phy_group), size = 3) + 
  stat_ellipse(level=0.9, linetype = 2, aes(color = basin)) +
  theme_bw()
test_nmds3

test_nmds4 <- ggplot(nmds2.merge[(nmds2.merge$time_point == "4"), ], aes(x=NMDS1, y=NMDS2, color = basin)) +
   geom_point(aes(color = basin, shape = phy_group), size = 3) + 
   stat_ellipse(level=0.9, linetype = 2, aes(color = basin)) +
   theme_bw()
test_nmds4

test_nmds5 <- ggplot(nmds2.merge[(nmds2.merge$time_point == "5"), ], aes(x=NMDS1, y=NMDS2, color = basin)) +
   geom_point(aes(color = basin, shape = phy_group), size = 3) + 
   stat_ellipse(level=0.9, linetype = 2, aes(color = basin)) +
   theme_bw()
test_nmds5
...
time.ord <- ordinate(PS.rare, method = 'MDS', distance = "bray")
time.MDS <- plot_ordination(PS.rare, time.ord) +
...
timepoint.nmds <- ggplot(nmds2.merge, aes(x=NMDS1, y=NMDS2, color = time_point)) +
  geom_point(aes(color = time_point), size = 3) + 
  stat_ellipse(level=0.9, linetype = 2) +
  theme_bw()
timepoint.nmds
  
nmds.lake <- ggplot(nmds2.merge, aes(x=NMDS1, y=NMDS2, color = lake)) +
...
phygroup.nmds <- ggplot(nmds2.merge, aes(x=NMDS1, y=NMDS2, color = phy_group)) +
...
dbRDA <- dbrda(bc_vegdist ~ longitude + latitude + elevation_m + DO_perc + chla_ug.L + 
                  cond, data = nmds2.merge, na.omit)
anova(dbRDA)
...
Abund <- aggregate(Abundance ~ time_point + lake + organism_water + phy_group + 
                     temp_C + Phylum + Class + Order + Family +
...
Facet <- ggplot(Abund, aes(x = time_point, y = Abundance, fill= Order)) +
...
stacked1 <- ggplot(data=Abund, aes(x = time_point, y = Abundance), fill = Order) +
  geom_bar(aes(fill = Order), stat ="identity", position ="fill") +
  facet_grid(col=vars(lake), scales="free")
stacked1
