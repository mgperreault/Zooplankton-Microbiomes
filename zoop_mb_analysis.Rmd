...
C.abund <- transform_sample_counts(glom, function(x)100* x / sum(x))
...
anova(lm(shannon ~ lake + time_point + sample_type + phy_group + basin, data = rare.a.diversity))
...
anova(lm(observed ~ lake + time_point + sample_type + phy_group + basin, data = rare.a.diversity))
...
mantel_test <- mantel.rtest(spatial_dist, bc.hell, nrepet=999)
mantel_test
...
# CAPSCALE
set.seed(10000)
capscale_result <- capscale(bc.hell ~ chla_ug.L + cond + DO_perc + pH + temp_C + DOC + elevation_m, dfun = "distance", data = nmds.merge.hell, distance = "bray")
...
CAPxMDS_data <- data.frame(CAP1 = cap_scores$CAP1, MDS1 = cap_scores$MDS1, time = sample_data(PS.hell)$time_point, lake = sample_data(PS.hell)$lake)
CAPxMDS <- ggplot(CAPxMDS_data, aes(x = CAP1, y = MDS1, color = lake)) +
  ...
  theme_bw()
CAPxMDS 
dev.copy(pdf, "phyloseq analysis/figures/CAPxMDS.pdf", height=4, width=10)
dev.off() 
...
Facet.hell <- ggplot(Abund.hell, aes(x = time_point, y = Abundance, fill= Class, color = Class)) +
  ...
  scale_color_manual(values = c("red4", "orangered", "gold", "lightgreen", "turquoise3", "seagreen4", "lightskyblue", "purple3", "skyblue4", "navy", "tan3", "royalblue3", "black", "antiquewhite2", "chartreuse3", "chocolate", "orchid", "lightpink"))
Facet.hell
dev.copy(pdf, "phyloseq analysis/figures/Facet.hell.pdf", height=6, width=10)
dev.off()
