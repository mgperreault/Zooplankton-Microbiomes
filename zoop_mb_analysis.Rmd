...
pseq <- microbiome::transform(PS.hell, "compositional")
ps1.fam.rel <-aggregate_rare(pseq, level = "Class", detection = 0.005, prevalence = 0.1)

p <- plot_composition(ps1.fam.rel,
                      average_by = "sample_type") + 
...
class.comp.plot <- print(p + scale_fill_brewer("Class", palette = "Paired") + theme_bw())
...
taxa_table <- tax_table(PS.hell)
taxa_class <- taxa_table[, "Class"]
unique_classes <- as.data.frame(unique(taxa_class))
...
nmds.merge.hell$basin <- as.factor(ifelse(nmds.merge.hell$lake == "Eastern Brook" | nmds.merge.hell$lake == "Serene" | nmds.merge.hell$lake == "Convict", "South", "North"))
...
rich.rare <- richness(otu_table(PS.hell), index ="observed")
div.rare <- diversity(otu_table(PS.hell), index="shannon")
...
rare.a.diversity <- cbind(rare.a.diversity, sample_data(PS.hell)[,c(3,4,7,8,9,10:19,22:24)])

# bell curve/ normally distributed, so we can use anova
hist(rare.a.diversity$shannon, main="Shannon diversity", xlab="", breaks=10)
shapiro.test(rare.a.diversity$shannon)


# lake and sample type are significant
anova(lm(shannon ~ lake + time_point + sample_type, data = rare.a.diversity))
shan.anova <- aov(lm(shannon ~ lake + time_point + sample_type + phy_group + basin, data = rare.a.diversity))
...
anova(lm(observed ~ lake + time_point + sample_type, data = rare.a.diversity))
rich.anova <- aov(lm(observed ~ lake + time_point + sample_type + phy_group + basin, data = rare.a.diversity))
TukeyHSD(rich.anova) # again, Serene is driving the significant difference

summary(lm(shannon ~  temp_C + DOC + elevation_m + chla_ug.L + pH + DO_perc + cond, data = rare.a.diversity))
summary(lm(observed ~  temp_C + DOC + elevation_m + chla_ug.L + pH + DO_perc + cond, data = rare.a.diversity))

# R squared
summary(lm(observed ~ chla_ug.L + pH, data = rare.a.diversity))

a.div.mod <- lm(shannon ~ temp_C + DOC + elevation_m + chla_ug.L + pH + DO_perc + cond, data = rare.a.diversity)
a.div.AIC <- MASS::stepAIC(a.div.mod, direction = "both")


rich.mod <- lm(observed ~ temp_C + DOC + elevation_m + chla_ug.L + pH + DO_perc + cond + basin, data = rare.a.diversity)
rich.AIC <- MASS::stepAIC(rich.mod, direction = "both")
rich.mod <- lm(observed ~ chla_ug.L + DO_perc, data = rare.a.diversity)




# summaryBy for averages and lm for regression model to conduct anova for significance
rare.sum_div <- summaryBy(shannon ~ sample_type, data = rare.a.diversity, FUN = c(mean, se))
rare.sum_rich <- summaryBy(observed ~ sample_type, data = rare.a.diversity, FUN = c(mean, se))

time_div <- summaryBy(shannon ~ time_point, data = rare.a.diversity, FUN = c(mean, se))
time_rich <- summaryBy(observed ~ time_point, data = rare.a.diversity, FUN = c(mean, se))
...
a.diversity.zoops <- cbind(a.diversity.zoops, sample_data(PS.zoops)[,c(7,9:19,22,23,24)])

# bell curve/ normally distributed, so we can use anova
hist(a.diversity.zoops$shannon, main="Shannon diversity", xlab="", breaks=10)
shapiro.test(a.diversity.zoops$shannon)

# running stepwise model selection
a.div.zoop.mod <- lm(shannon ~ lake + time_point + phy_group + temp_C + DOC + elevation_m + elev.cat + basin + chla_ug.L + pH + DO_perc + cond, data = a.diversity.zoops)
a.div.zoop.AIC <- MASS::stepAIC(a.div.zoop.mod, direction = "both")

rich.zoop.mod <- lm(observed ~ lake + time_point + phy_group + temp_C + DOC + elevation_m + chla_ug.L + pH + DO_perc + cond, data = a.diversity.zoops)
rich.zoop.AIC <- MASS::stepAIC(rich.zoop.mod, direction = "both")
...
bargraph.CI(chla_ug.L, observed, data = rare.a.diversity,
                     ...
                     col = "gray40", angle = 45, cex.names = 1,
                     density = c(0,20), legend = TRUE)
...
# bell curve/ normally distributed, so we can use anova
hist(water.a.diversity$shannon, main="Shannon diversity", xlab="", breaks=10)
shapiro.test(water.a.diversity$shannon)
...
Bacteroidia <- subset(Abund.hell, Class == "Bacteroidia")
...
Bacteroidia <- merge(Bacteroidia, sample_data(PS.hell), by = "row.names")
Anova(lm(Abundance.sum ~ lake + time_point + phy_group, data = Bacteroidia))
# nothing significant

gammaproteobacteria <- subset(Abund.hell, Class == "Gammaproteobacteria")
...
FitAll <- lm(Abundance.sum ~ temp_C + DO_perc + pH + DOC + elevation_m + cond + chla_ug.L, data = gammaproteobacteria)
# Dissolved oxygen and chla are important
step(FitAll, direction = "both")


alphaproteobacteria <- subset(Abund.hell, Class == "Alphaproteobacteria")
...
# lake is important
FitAll <- lm(Abundance.sum ~ temp_C + DO_perc + pH + DOC + elevation_m + cond + chla_ug.L, data = alphaproteobacteria)
# Dissolved oxygen and chla are important
step(FitAll, direction = "both")
# cond and chla


actinobacteria <- subset(Abund.hell, Class == "Actinobacteria")
...
actinobacteria <- merge(actinobacteria, metadata_500, by = "row.names")
Anova(lm(Abundance.sum ~ lake + time_point + phy_group, data = actinobacteria))
# lake is important
FitAll <- lm(Abundance.sum ~ temp_C + DO_perc + pH + DOC + elevation_m + cond + chla_ug.L, data = actinobacteria)
# Dissolved oxygen and chla are important
step(FitAll, direction = "both")
# cond and chla
...
# subset by lake so that we can look at the env. effects through TIME in EACH LAKE
env.hell.serene <- sample_data(PS.hell)[sample_data(PS.hell)$lake == "Serene" ,c(10:16,18,19)]

# have to actually go back, trace my steps to run an nmds on just serene, and just convict, etc. to get a matching nmds object
nmds.hell.serene

# run the envfit function
fit.env.hell <- envfit(nmds.hell, env.hell, na.rm = T)
fit.env.hell.serene <- envfit(nmds.hell, env.hell.serene, na.rm = T)
...
dist.df <- as.data.frame(cbind(spatial_dist_km, bc_500))
##############
#Generate plots for all samples combined
dist_plot <- ggplot(dist.df, aes(x=spatial_dist_km, y=bc_500)) +
...
plot(capscale_model)
# BUT chla and DOC also seem important
alt_model <- capscale(bc.hell ~ temp_C + DOC + chla_ug.L, data = nmds.merge.hell, distance = "bray")
anova(alt_model)
plot(alt_model)
...
# plot
overall.comp <- ggplot(Abund.final, aes(x=phy_group, y=rel_abund, fill=Class, color=Class)) +
  geom_bar(aes(fill=Class, color=Class), stat="identity", position="fill") +
...
phyla.hell <- ggplot(Abund.hell, aes(x = time_point, y = Abundance, fill= Phylum, color = Phylum)) +
  geom_bar(aes(fill = Phylum, color = Phylum), stat ="identity", position = "fill") +
  facet_grid(cols = vars(lake), rows = vars(phy_group)) + xlab("Sampling Time Point") +  ylab("Relative Abundance (%)") +
  theme_bw() + theme(axis.text.x=element_text(size=12)) +
  theme(axis.text.y=element_text(size=10)) +
  theme(strip.text.y = element_text(size = 15)) +
  scale_fill_manual(values = c("red4", "orangered", "gold", "lightgreen", "turquoise3", "seagreen4", "lightskyblue", "purple3", "skyblue4", "navy", "tan3", "royalblue3", "black", "antiquewhite2", "chartreuse3", "chocolate", "orchid", "lightpink")) +
  scale_color_manual(values = c("red4", "orangered", "gold", "lightgreen", "turquoise3", "seagreen4", "lightskyblue", "purple3", "skyblue4", "navy", "tan3", "royalblue3", "black", "antiquewhite2", "chartreuse3", "chocolate", "orchid", "lightpink"))
phyla.hell



time.hell <- ggplot(Abund.hell, aes(x=time_point, y=Abundance, fill=Class, color=Class)) +
  geom_bar(aes(fill=Class, color=Class), stat="identity", position="fill") +
  xlab("Sampling Time Point") + ylab("Relative Abundance (%)") + theme(axis.text.x=element_text(size=12)) +
  theme(axis.text.y=element_text(size=10)) +
  theme(strip.text.y = element_text(size = 15)) + theme_bw() +
  scale_fill_manual(values = c("red4", "orangered", "gold", "lightgreen", "turquoise3", "seagreen4", "lightskyblue", "purple3", "skyblue4", "navy", "tan3", "royalblue3", "black", "antiquewhite2", "chartreuse3", "chocolate", "orchid", "lightpink")) +
  scale_color_manual(values = c("red4", "orangered", "gold", "lightgreen", "turquoise3", "seagreen4", "lightskyblue", "purple3", "skyblue4", "navy", "tan3", "royalblue3", "black", "antiquewhite2", "chartreuse3", "chocolate", "orchid", "lightpink"))
time.hell
dev.copy(pdf, "phyloseq analysis/figures/time.hell.pdf", height=6, width=10)
dev.off()

sample.type.hell <- ggplot(Abund.hell, aes(x=phy_group, y=Abundance, fill=Class, color=Class)) +
  geom_bar(aes(fill=Class, color=Class), width = 0.75, stat="identity", position="fill") +
  xlab("Sample Type") +  ylab("Relative Abundance (%)") + theme(axis.text.x=element_text(size=12)) +
  theme(axis.text.y=element_text(size=10)) +
  theme(strip.text.y = element_text(size = 15)) + theme_bw() +
  scale_fill_manual(values = c("red4", "orangered", "gold", "lightgreen", "turquoise3", "seagreen4", "lightskyblue", "purple3", "skyblue4", "navy", "tan3", "royalblue3", "black", "antiquewhite2", "chartreuse3", "chocolate", "orchid", "lightpink")) +
  scale_color_manual(values = c("red4", "orangered", "gold", "lightgreen", "turquoise3", "seagreen4", "lightskyblue", "purple3", "skyblue4", "navy", "tan3", "royalblue3", "black", "antiquewhite2", "chartreuse3", "chocolate", "orchid", "lightpink"))
sample.type.hell
dev.copy(pdf, "phyloseq analysis/figures/sample.type.hell.pdf", height=6, width=10)
dev.off()

sample.type.order <- ggplot(Abund.hell, aes(x=phy_group, y=Abundance, fill=Order, color=Order)) +
  geom_bar(aes(fill=Order, color=Order), width = 0.75, stat="identity", position="fill") +
  xlab("Sample Type") +  ylab("Relative Abundance (%)") + theme(axis.text.x=element_text(size=12)) +
  theme(axis.text.y=element_text(size=10)) +
  theme(strip.text.y = element_text(size = 15)) + theme_bw() +
  ...
  scale_color_manual(values = c('turquoise','pink2', 'tan', 'orange', 'cornflowerblue', 
                ...
                "darkolivegreen1" ,"tan2" ,   "tomato3" , "#7CE3D8","gainsboro", "gold2", "chocolate", "darkgreen", "dodgerblue", "burlywood3", 'hotpink', 'yellow3', 'blue', 'pink', 'brown3'))
sample.type.order
dev.copy(pdf, "phyloseq analysis/figures/sample.type.order.pdf", height=6, width=10)
dev.off()
