...
log.DOC.plot<-ggplot(lakeDOC.df, aes(x=Time.Point, y=DOC_values, group=Lake, color=Lake)) +
  scale_y_log10() + geom_line() + scale_color_manual(values = c("royalblue", "darkorchid4", "chartreuse2", "orange2", "violet", "darkgreen")) +
...
chla.plot<- ggplot(chla.df, aes(x= Time.Sample, y=Chlorophyll.1..ug.L, group=Lake, color=Lake)) + geom_line() + scale_color_manual(values = c("royalblue", "darkorchid4", "chartreuse2", "orange2", "violet", "darkgreen")) + xlab("Sampling Time Point") + theme(axis.text = element_text(size=7)) +  ylab(expression(paste("Chl-a", ~(mu*g~ L^-1), sep = "")))
chla.plot
...
conductivity.plot<-ggplot(lakeCond.df, aes(x=Time.Point, y=cond, group=Lake, color=Lake)) +
  geom_line() + theme(axis.text.x = element_text(size = 10)) +
...
avg.temp <- c(18,18.5,19,17.5,18,18.5,22.5,22,19.5,16,18.5,21.5,21.5)
rain.total <- c(0,0,0,0,0,0,0,0,0.6,1.6,2.2,0,0)
TimePoint <- c('June-25', 'June-26', 'June-29', 'July-6', 'July-7', 'July-9', 'July-21', 'July-22', 'Aug-4', 'Aug-5', 'Aug-9', 'Aug-21', 'Aug-23')
date <- as.Date(c("2022-06-25", "2022-06-26", "2022-06-29", "2022-07-06", "2022-07-07", "2022-07-09", "2022-07-21", "2022-07-22", "2022-08-04", "2022-08-05", "2022-08-09", "2022-08-21", "2022-08-23"))
weather <- data.frame(date = date, temperature = avg.temp, precipitation = rain.total)
weather <- weather[weather$date > as.Date(c("2022-06-25", "2022-06-26", "2022-06-29", "2022-07-06", "2022-07-07", "2022-07-09", "2022-07-21", "2022-07-22", "2022-08-04", "2022-08-05", "2022-08-09", "2022-08-21", "2022-08-23")), ] %>%
  filter(variable == "weather$temperature" | variable == "weather$precipitation")


weather.plot <- ggplot(weather, aes(x=fct_inorder(date))) +
  geom_line(aes(y=temperature, group=1, color="temperature"), linewidth=1.5) + 
  geom_line(aes(y=precipitation, group=1, color="precipitation"), linewidth=1.5) +
  scale_x_date(
    date_breaks = "1 day",
    date_labels = "%Y-%m-%d"
  ) +
  scale_color_manual(values = c("Temperature" = "orangered", "Precipitation" = "steelblue")) +
  labs(x="Sampling Date", y="Average Air Temperature (°C)") +
  theme_bw()+ theme(axis.text.x = element_text(angle = 90))
  
weather.plot <- weather.plot + scale_y_continuous(
  sec.axis = sec_axis(~./1, name = "Total Daily Precipitation (mm)"),
  limits = c(0,max(weather$Temperature, max(weather$Precipitation)))
)
 weather.plot 
weather.plot
...
weather.plot <- ggplot(weather, aes(x = date)) +
  geom_line(aes(y = temperature, color = "temperature"), linewidth = 1.2, linetype = "solid") +
  geom_line(aes(y = precipitation, color = "precipitation"), linewidth = 1.2, linetype = "solid") +
  labs(x = "Date", y = "Air Temperature (°C)", title = NULL) + 
  scale_color_manual(
    values = c("precipitation" = "steelblue", "temperature" = "orangered"), breaks = c("temperature", "precipitation")
  ) +
